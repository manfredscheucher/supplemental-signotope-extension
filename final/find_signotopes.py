...
parser.add_argument("--numplussigns","-p",type=int,help="number of + signs")
...
parser.add_argument("--fliples",action='store_true',help="make assumptions to fliples")
...
if args.numplussigns:
	all_variables += [('count+',(I,k)) for I in combinations(N,rank) for k in range(p+1)]
...
def var_countp(*L): return var(('count+',L))
...
if args.numplussigns:
	...
	for I in combinations(N,rank):
		...
		# assert count 
		constraints.append([var_countp(I,k) for k in range(p+1)])
		for k,l in combinations(range(p+1),2):
			constraints.append([-var_countp(I,k),-var_countp(I,l)])

		if prevI != None:
			constraints.append([-var_countp(prevI,p),-var_sign(*I)])
			for k in range(p):
				constraints.append([-var_countp(prevI,k),-var_sign(*I),+var_countp(I,k+1)])

			for k in range(p+1):
				constraints.append([-var_countp(prevI,k),+var_sign(*I),+var_countp(I,k)])
				
		else:
			constraints.append([+var_sign(*I),+var_countp(I,0)])
			if p > 0:
				constraints.append([-var_sign(*I),+var_countp(I,1)])
			else:
				constraints.append([-var_sign(*I)])

		prevI = I
	constraints.append([var_countp(prevI,p)])
...
if args.fliples:
	assert(n == 2*rank)
	print ("(*) prescribe fliples")
	A = tuple(range(rank//2)) 
	B = tuple(range(rank//2,rank))
	C = tuple(range(rank,3*rank//2))
	D = tuple(range(3*rank//2, 2*rank))
	List_fliples = [A+B, A+C, B+D, C+D] 
	for I in combinations(N,rank):
		if I in List_fliples:
			constraints.append([+var_flipable(*I)])
#		else: 
#			constraints.append([-var_flipable(*I)])
